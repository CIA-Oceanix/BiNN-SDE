...
#    cov=np.zeros((T,n*n))
    ...
    if (DA.method =='GKF'):
        class x_hat:
            ...
            time = yo.time;
        Pf = np.zeros([T,n,n]);
        x_hat.values[0,:]=DA.xb
        for k in tqdm(range(1,T)):
            # update step (compute forecasts)            
            xf = DA.prediction_model.predict(np.reshape(x_hat.values[k-1,:],(1,DA.xb.shape[0])));
            Ef = DA.cov_model.predict(np.reshape(x_hat.values[k-1,:],(1,DA.xb.shape[0])))
            np.fill_diagonal(Pf[k,:,:],Ef[0,:])
            # analysis step (correct forecasts with observations)          
            i_var_obs = np.where(~np.isnan(yo.values[k,:]))[0];            
            if (len(i_var_obs)>0):                
                eps = np.random.multivariate_normal(np.zeros(len(i_var_obs)),DA.R[np.ix_(i_var_obs,i_var_obs)],1);
                ...
                d = np.repeat(yo.values[k,i_var_obs][np.newaxis],1,0)+eps-yf;
                x_hat.values[k,:] = xf + np.dot(d,K.T);    
            else:
                x_hat.values[k,:] = xf;            

    elif (DA.method =='AnEnKF' or DA.method =='AnEnKS'):
